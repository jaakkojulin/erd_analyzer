...
void create_plotfile(depthfile_t *depthfiles, element_t *elements, rgbcolor_t *colors, depth_scale_t *depthscale, plot_options_t *plot_options, char *headers_filename, char *filename) {
    ...
    for(this=depthfiles; this != NULL; this=this->next) {
        ...
        if(this->A) {
            fprintf(out, "%s '%s' index %i using 1:($2*SCALE) with lines lt 1 lw %g lc rgbcolor \"#%02x%02x%02x\" title \"%i-%s\"%s", i?"":"plot", plotdatafilename, i, plot_options->linewidth, this->color->r, this->color->g, this->color->b, this->A, elements[this->Z].name, (i<n-1)?", \\\n":"\n");
        } else {
            fprintf(out, "%s '%s' index %i using 1:($2*SCALE) with lines lt 1 lw %g lc rgbcolor \"#%02x%02x%02x\" title \"%s\"%s", i?"":"plot", plotdatafilename, i, plot_options->linewidth, this->color->r, this->color->g, this->color->b, elements[this->Z].name, (i<n-1)?", \\\n":"\n");
        }
        i++;
    }
    for(this=depthfiles; this != NULL;  this=this->next) {
